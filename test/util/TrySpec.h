...
private slots:
    void constructor_and_apply_should_properly_instantiate_Try() {
        ...
        Try<int> t2(QSharedPointer<const Exception> (new Exception("Test exception."))); QVERIFY_EXCEPTION_THROWN(t2.get(), Exception *); QCOMPARE(t2.isFailure(), true);
        ...
        Try<int> t5(applyTry<int>([]() -> int { throw new Exception(); }));
    ...
    void flatMap_should_ignore_the_function_after_failure() {
        ...
        Try<QString> actual = t1
                ...
                .flatMap<int>([](const int &t) -> Try<int> { throw new Exception(); })
